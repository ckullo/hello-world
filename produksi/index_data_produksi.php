...
<script type="text/Javascript">
    ...
    function un_approve_cw_conf(val) {
        alert(val);
        var cek = confirm('Are you sure to UN - APPROVE ?');
        if (cek) {
            un_approve_cw(val);
        }
    }

    function un_approve_cw(val) {

//alert(val);

        $.post('inputan_baru.php?act=un_approve_cw&list_antrian_app=' + val, '', function (respon) {
            if (respon == 'sukses') {
                alert('success');
            } else {
                alert(respon);
...
//alert(val);
    ...
    function edit_packing_all(val) {	//add_packing_all(val);
...
//	alert('txt_list_order :' + txt_list_order);
    ...
    function show_hide_div(nilai) {
        ...
        //var val = "'"+val+"'"
        if (nilai == '0') {
            var divnya = "div_nilai_rps";
        } else if (nilai == '1') {
            var divnya = "div_belum_pack";
        }
...
//alert(row);
    ...
    function print_besar_conf() {
        ...
        if (list_antrian != "") {
            ...
            if (cek) {
                cetak_label_besar(list_antrian, copies);
            }
        }
    }

    function print_besar_iljin_conf_2() {
        var list_antrian = $("#list_antrian").val();
        //alert (list_antrian);
        var copies = $("#copies").val();
        //alert(copies);
        if (list_antrian != "") {
            var cek = confirm('Anda yakin untuk Cetak Label ILJIN ?\nPastikam Printer sudah siap untuk Cetak Label ILJIN ! ');
            if (cek) {
                cetak_label_iljin(list_antrian, copies);
            }
        }
    }
...
//	alert (list_antrian);
    ...
    function print_besar_iljin_conf(row) {
        //var list_antrian = $("#list_antrian").val();
        var copies = $("#copies_inputan_baru").val();
//alert(row);
        var list_antrian = $('#var_print_' + row).val();
//	alert (list_antrian);
    ...
    function cetak_label_iljin(val, copies) {
        $.post('script_data_produksi.php?act=cetak_label_iljin&id=' + val + '&copies=' + copies, '', function (respon_save) {
            if (respon_save == 'sukses') {
            } else {
            }
        });
...
//$lemparan_approve = "'".$offset.'|'.$id_tr_order."'";
    ...
    function cancel_conf(val) {
        ...
        var cek = confirm('Anda yakin untuk CANCEL Order No.  ' + order_no + ' ini ?');
        if (cek) {
            cek_isi(val);
        }
    }
...
//alert('view_history');
	...
	{if(lot[i].value != ''){jum_lot++;}}*/
        ...
        if (jum_jb < tinggi) {
            alert('JB belum di isi!');
            return false;
        }
        if (jum_cs < tinggi) {
            alert('CS belum di isi!');
            return false;
        }
        if (jum_lebar < tinggi) {
            alert('Lebar belum di isi!');
            return false;
        }
        if (jum_panjang < tinggi) {
            alert('Panjang belum di isi!');
            return false;
        }
        if (jum_mat < tinggi) {
            alert('Material Code belum di isi!');
            return false;
        }
...
//alert(pilihan);
    ...
    function update_all_partai_new(val) {
        //    alert(val);
        var myvar = val.split('zzz');
        var no_ = myvar[0];

        var text_nya = "text_counter_awal_" + no_;
        var e = $("#" + text_nya);
        var text_counter_awal = e.val();

        var text_nya = "text_counter_akhir_" + no_;
        var e = $("#" + text_nya);
        var text_counter_akhir = e.val();

        var text_nya = "text_waste_edge_" + no_;
        var e = $("#" + text_nya);
        var text_waste_edge = e.val();

        var text_nya = "text_waste_reclaime_" + no_;
        var e = $("#" + text_nya);
        var text_waste_reclaime = e.val();

//alert(e.val());
//var cbo_program = $("#text_counter_awal").val();
        /*var txt_menuid = $("#txt_menuid").val();
			var id_tr_pk = myvar[1];
...
var grade = 0;*/
//alert(pilihan);
        $.post('inputan_baru.php?act=save_partai_new&txt_lemparan=' + val + '&text_counter_awal=' + text_counter_awal + '&text_counter_akhir=' + text_counter_akhir + '&text_waste_edge=' + text_waste_edge + '&text_waste_reclaime=' + text_waste_reclaime, $("#form_add_partai").serialize(), function (respon_save) {
            //$.post('inputan_baru.php?act=save_partai_new&txt_lemparan='+val,$("#form_add_partai").serialize(),function(respon_save){
            if (respon_save == 'sukses') {
                alert('Success');
                // parent.jQuery.fancybox.close();
...
//$("#txt_sembunyi").value = status;
        ...
        $.post('script_data_produksi.php?act=save_produksi&lemparan=' + val, $("#form_view_sub").serialize(), function (respon_save) {
            ...
            Loaddiv('div_data', 'script_data_produksi.php', 'act=show_table&offset=' + offset + '&txt_sembunyi=' + txt_sembunyi + '&periode=' + periode + '&txt_menuid=' + txt_menuid + '&cbo_program=' + cbo_program + '&id_m_line=' + id_m_line);
            $.post('script_data_produksi.php?act=add_order&lemparan=' + val, '', function (respon_) {
                $.fancybox(respon_);
            });
...
INNER JOIN m_group_line b on a.id_m_group_line = b.id_m_group_line
				...
				";
                    ...
                    <td align="left">Periode :
                        <select name="cbo_periode" id="cbo_periode" class="combobox"
                                onchange="show_blth_log(this.value)">
                    ...
                    <td rowspan="3" align="right"><label for="cbo_pilihan">Status RPS(R)</label>
                        <select name="cbo_pilihan" id="cbo_pilihan" class="combobox"
                                onchange="show_blth_log(this.value)">
                        ...
                        <!-- <input type="button" name="button_add" id="button_search" value="Search" class="button" onclick="search_data()" />-->
                    </td>
                </tr>
                <tr>
                    <td align="left">Program :
                        <select name="cbo_program" id="cbo_program" class="combobox"
                                onchange="show_blth_log(this.value)">
                            <option value="0">- All -</option>
